...
drop view  Student_Registration;
drop view New_Admissions;
drop view  Student_History;
drop view  Student_Registration_History;
drop View Student_Registration;
  ...
  BEGIN
   ...
   if counter = 5 then
      ...
      class_id11.cl_id:=class_id11.cl_id+1;
       ...
       insert into CLASS (class_id,section,class_name) values(class_id1,section,'1');
       end if;
     IF counter is NULL then
        counter :=1;
        :new.class_count := counter;
        :new.class_id := class_id1;
        update class set class_strength = :new.class_count where class_id = :new.class_id;
      ELSE
      counter:=counter+1;
      :new.class_count := counter;
      :new.class_id := class_id1;
  ...
  BEGIN
   ...
   if counter = 5 then
     ...
     class_id22.cl_id := class_id22.cl_id+1;
       ...
       insert into class (class_id,section,class_name) values (class_id2,section,'2');
    end if;
     IF counter is NULL then
        counter :=1;
        :new.class_count := counter;
  ...
  BEGIN
   ...
   if counter = 5 then
      ...
      class_id33.cl_id:=class_id33.cl_id+1;
       ...
       insert into class (class_id,section,class_name) values(class_id3,section,'3');
    end if;
     IF counter is NULL then
        counter :=1;
        :new.class_count := counter;
  ...
  BEGIN
   ...
   if counter = 5 then
      ...
      class_id44.cl_id:=class_id44.cl_id+1;
       ...
       insert into class(class_id,section,class_name) values (class_id4,section,'4');
    end if;
     IF counter is NULL then
        counter :=1;
        :new.class_count := counter;
  ...
  BEGIN
   ...
   if counter = 5 then
      ...
      class_id55.cl_id:=class_id55.cl_id+1;
       ...
       insert into class(class_id,section,class_name) values (class_id5,section,'5');
    end if;
     IF counter is NULL then
        counter :=1;
        :new.class_count := counter;
...
insert into Class(class_id,Section,Class_Name,class_title)
values (100,'A','1','viking');
insert into Class(class_id,Section,Class_Name,class_title)
values (200,'A','2','haxor');
insert into Class(class_id,Section,Class_Name,class_title)
values (300,'A','3','cyber');
insert into Class(class_id,Section,Class_Name,class_title)
values (400,'A','4','shaphar');
insert into Class(class_id,Section,Class_Name,class_title)
values (500,'A','5','ghareeb log');

insert into course (co_name,co_type,co_dur,age,status,fee)
values ('algebra','m/f','1',4,1,10000);
insert into course (co_name,co_type,co_dur,age,status,fee)
values ('algebra','m/f','1',5,1,20000);
insert into course (co_name,co_type,co_dur,age,status,fee)
values ('algebra','m/f','1',8,1,30000);
insert into course (co_name,co_type,co_dur,age,status,fee)
values ('algebra','m/f','1',13,1,40000);
insert into course (co_name,co_type,co_dur,age,status,fee)
values ('algebra','m/f','1',15,1,50000);
commit;
...
select unique r.s_rollnumber, s.s_name, r.r_date, r.class_ID, &I_N_A New_Admissions_Check
from student s, Registration r
where s.s_rollnumber = r.s_rollnumber and (r.r_date) > &Date_1 and  (r.r_date) < &Date_2; 

--selects students who changed their class
create view Class_Change as
select s.s_name, rgm.s_rollnumber, rgm.class_id, lc.cl_id, rgm.New_Admissions_Check
...
select * from class_change where New_Admissions_Check = '0';

--selects all the New Admissions
create View New_Admissions as
select s_rollnumber from Registration_Gap_Months
minus
select s_rollnumber from Class_Change;

--shows all new admissions + Class Changed
select s.s_name, s.s_rollnumber, rgm.New_Admissions_Check 
from Student s ,New_Admissions na , Registration_Gap_Months rgm
where s.s_rollnumber = na.s_rollnumber and rgm.New_Admissions_Check = '1'
union
select s_name, s_rollnumber, cc.New_Admissions_Check
from Class_Change cc
where cc.New_Admissions_Check = '1';
...
--Query 10 for specific Student

create View Student_Registration as
  select s.s_name, rh.s_rollnumber, rh.cl_id
from Registration_History rh, student s
where rh.s_rollnumber = &id and rh.s_rollnumber = s.s_rollnumber;


create view Student_Registration_History as
select unique s.s_name, r.s_rollnumber, r.class_id
from Registration r, Student_Registration rh ,Student s
where r.s_rollnumber = rh.s_rollnumber and rh.cl_id != r.class_Id and r.s_rollnumber = s.s_rollnumber;

select sr.s_name, sr.s_rollnumber, sr.cl_id from Student_Registration sr, Student_Registration_History srh where sr.s_rollnumber = srh.s_rollnumber;
select  * from Student_Registration_History;


--Query 10 shows all students who changed their class
select s.s_name, rh.s_rollnumber, rh.cl_id 
from Registration_History rh, Student s
where rh.s_rollnumber = s.s_rollnumber
union
select s.s_name, r.s_rollnumber, r.class_id 
from Registration r, Registration_History rh ,Student s
where r.s_rollnumber = rh.s_rollnumber 
and r.class_Id != rh.cl_id
and r.s_rollnumber = s.s_rollnumber;

--Query 13
select rh.s_rollnumber
from Registration_History rh
where (MONTHS_BETWEEN(sysdate,rh.R_Date)>6)
minus
select r.s_rollnumber
from Registration r;
